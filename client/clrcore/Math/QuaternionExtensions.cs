...
namespace CitizenFX.Core
{
    internal static class QuaternionExtensions
    {
        internal static Vector3 ToRotation(this Quaternion q)
        {
            ...
            return new Vector3(MathHelper.RadianToDegree(pitch), MathHelper.RadianToDegree(roll), MathHelper.RadianToDegree(yaw));
        }

        internal static Quaternion FromRotation(Vector3 value)
        {
            float rotX = MathHelper.DegreeToRadian(value.X);
            float rotY = MathHelper.DegreeToRadian(value.Y);
            float rotZ = MathHelper.DegreeToRadian(value.Z);
