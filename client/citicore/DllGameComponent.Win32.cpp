...
Component* DllGameComponent::CreateComponent()
{
	...
	if (!createComponent)
	{
		...
		{
			additionalInfo = " You likely overwrote scripthookv.dll from FiveM with a non-MV version of it. Delete caches.xml to restore from this heinous act.";
		}
...
void DllGameComponent::ReadManifest()
{
	...
	else
	{
		...
		if (m_path.find(L"scripthookv") != std::string::npos)
		{
			additionalInfo = " You likely overwrote scripthookv.dll from MultiFive with a non-MV version of it. Delete caches.xml to restore from this heinous act.";
		}
